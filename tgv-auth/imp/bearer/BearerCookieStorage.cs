    ...
    public BearerSession? GetUserSession(Context ctx)
    {
        ...
        catch (TokenNotYetValidException)
        {
            ctx.Logger.Debug("Token is not valid yet");
             Statics.GetMetric().CreateCounter<int>("bearer_cookie_token_not_yet_valid", description: "Bearer Token is not valid yet")
                .Add(1, ctx.ToTagsFull().With("algo", _algo));
        }
        catch (TokenExpiredException)
        {
            ctx.Logger.Debug("Token has expired");
             Statics.GetMetric().CreateCounter<int>("bearer_cookie_token_expired", description: "Bearer Token is expired")
                .Add(1, ctx.ToTagsFull().With("algo", _algo));
        }
        catch (SignatureVerificationException)
        {
            ctx.Logger.Debug("Token has invalid signature");
             Statics.GetMetric().CreateCounter<int>("bearer_cookie_token_invalid_signature", description: "Bearer Token has invalid signature")
                .Add(1, ctx.ToTagsFull().With("algo", _algo));
        }
        catch (Exception ex)
        {
            ctx.Logger.Error("Unknown error: {ex}", ex);
             Statics.GetMetric().CreateCounter<int>("bearer_cookie_token_unknown_error", description: "Bearer Token unknown error during validation")
                .Add(1, ctx.ToTagsFull().With("algo", _algo));
        }
